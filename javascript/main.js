...
const state = {
	...
	*/
	refreshGifs: (functionToBeCalledWithDataIsBack) => {
		...
		// const apiEndpoint = `http://api.giphy.com/v1/gifs/search?api_key=Bww6ZMR3jh0fLIFrZXXp2hhxOre9ni2g&q=${state.searchQuery}`
...
const start = Date.now();
        return vanillaGet(apiEndpoint)
			.then(data => {
				...
				for (let i = 0; i < data.data.length; i++) {
					const gifUrl = data.data[i].images.downsized_medium.url;
					state.youTubeVideo.push(youTubeVideoUrl)
				}
...
const render = () => {
	...
	// to this search query
	hint.innerHTML = '';
	youTubeContainer.innerHTML = `<div id="loading"></div>`;
	searchBtn.setAttribute('disabled', 'disabled');
	searchQueryInp.setAttribute('disabled', 'disabled');
	...
	state.refreshGifs()
		.then(() => {
			...
			youTubeContainer.innerHTML = str;
		});
